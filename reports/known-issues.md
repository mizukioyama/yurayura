...
- タイトル・開催期間を含む`.header`本体は左側に残し、ナビゲーションと収納時のハンバーガーだけを右側へ配置しています。公開375／768／1280pxで確認済みです。
- タイトル領域とメニューは上下に分離したままです。公開版の確認はChromeのIABエミュレーションによる375／768／1280pxで、実ブラウザのアドレスバーやOSの表示領域差は含みません。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボードのフォーカス順・可視性は未確認です。

## Header text persistence on scroll (2026-09-15)

- スクロール後も`.header`内のタイトル・開催期間を表示し、`.header-list`だけを収納する構成にしています。公開375／768／1280pxで確認済みです。
...
- `481px〜1199px`では3項目をスマホと同じ縦積みにし、各項目を100%幅にしています。`480px以下`の既存全幅指定も維持しています。
- `.header`本体は左側のまま、右側メニューの`.header-list`だけを同じ`--site-header-menu-top`で開始するようにしています。
- 公開TopのPC幅、公開Concept／Galleryの表示は確認済みです。タブレット規則はソースで確認済みですが、実機表示は未確認です。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボード操作とフォーカス順は未確認です。

## Header left alignment restoration (2026-09-15)

- `.header`全体とメニューを左端基準へ戻し、タイトル領域とメニューの上下分離は維持しています。公開375／768／1280pxで確認済みです。
- 公開版の確認はChromeのIABエミュレーションによる375／768／1280pxです。実ブラウザのアドレスバーやOSの表示領域差は含みません。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボードのフォーカス順・可視性は未確認です。

## Whole Header right-edge alignment (2026-09-14)

- `.header`全体（タイトル・開催期間・ナビゲーション）をTop／Concept／Galleryで同じレスポンシブな右端へ配置しました。公開375／768pxで右端一致と横方向オーバーフローなしを確認済みです。
- Galleryは固有CSSの後勝ち上書きを追加し、`gallery.css`のキャッシュバスターをv3へ更新しています。
- 公開版の確認はChromeのIABエミュレーションによる375／768pxです。実ブラウザのアドレスバーやOSの表示領域差は含みません。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボードのフォーカス順・可視性は未確認です。

## Header and menu overlap correction (2026-09-15)

- タイトル・開催期間と縦書きメニューを上下に分離し、Top／Concept／Galleryで重ならない構成にしました。公開375／768pxとTop 1280pxで確認済みです。
- 右端基準と横方向オーバーフローなしは維持しています。Gallery固有CSSにも同じルールを適用し、キャッシュバスターをv4へ更新しました。
- 公開版の確認はChromeのIABエミュレーションによる375／768／1280pxです。実ブラウザのアドレスバーやOSの表示領域差は含みません。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボードのフォーカス順・可視性は未確認です。
...
- デスクトップ幅のみ、アクセス直下の住所・問合せ・営業時間を`flex: 1 1 0`で均等伸縮させています。タブレット／スマホは既存の縦積みです。
- 公開版の確認はChromeのIABデスクトップ表示です。実機のアドレスバー・OS表示領域・フォント描画差は含みません。
- 実機スマートフォン／タブレット、Safari・Firefox・Edgeなど主要ブラウザ、キーボード操作とフォーカス順は未確認です。
