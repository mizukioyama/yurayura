...
Artistsはスライドショーから画像付きギャラリーカードへ変更しました。旧スライダー用の`memberSlider`/`memberTrack`は対象HTMLにないため、`slide.js`は対象なしで終了します。Viewボタンの既存幅・背景ルールは維持しています。
...
Concept/Artistsは専用のclamp値で上下paddingをさらに縮小しています。Artistsの背景画像コンテナは非表示で、カード画像付きギャラリーを表示します。

Conceptは意図的に共通sectionより広い上下余白を設定し、展示コンセプトの静かな見せ方を優先しています。

`main.css`には`v=20260909-artists-gallery-layout`のキャッシュバスターを付けています。
