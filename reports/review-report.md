...
Artistsのスライドショー表示領域は基本を `width: 100%` とし、スマホではセクション左右余白分を `calc(100% + 80px)` で補正して画面幅に揃えました。さらに各 `.card` を `flex-basis: 100%` とし、1枚のスライドも画面幅に揃えました。無限ループ用のトラックと既存アニメーションは維持しています。
...
| スライドショー幅 | PASS | 320〜1440pxで表示領域・トラック・1枚のカードが各viewport幅、内部トラックの無限ループを維持 |
