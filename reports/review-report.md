...
FAQの`faq.js`を、確実な初期化・開閉状態の同期・`aria-expanded`/`aria-controls`設定に対応させました。開く項目を1つに絞り、クリックした項目は再クリックで閉じられます。
...
| FAQアコーディオン | PASS | `.faq-question`のクリックで開閉し、`is-open`とARIA状態を同期。複数項目を同時に開かない |
