...
FAQの`faq.js`を、確実な初期化・開閉状態の同期・`aria-expanded`/`aria-controls`設定に対応させました。トップレベルFAQと購入内の「購入方法・支払い方法・発送について・領収書について」を個別のアコーディオンにし、各階層で開く項目を1つに絞りました。
...
| FAQアコーディオン | PASS | `.faq-question`と購入内の`.faq-sub-question`をJSで開閉し、各階層で`is-open`とARIA状態を同期。複数項目を同時に開かない |
